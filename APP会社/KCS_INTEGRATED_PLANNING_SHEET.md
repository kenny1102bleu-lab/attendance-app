...
graph TD
    ...
    subgraph "【音声＆映像出力 (２台のパソコン連携)】"
        H["パイソン制御<br>(Python)"]
        I["オービーエス<br>(OBS Studio)"]
        J["ボイスボックス & アールブイシー<br>(VOICEVOX / RVC)"]
    end
...
### ③ 🛍️ ユーチューブ・ライブ（YouTube LIVE）配信での「ミモミＴシャツＰＲ」動画切り替え

* **PRトリガー**: 「ミモミ」「Tシャツ」「アパレル」「服」などのワードを検知した瞬間、自動で**【斜め下・概要欄指差し動画（エムピーフォー）】**へ切り替え。
  * *セリフ*: 「ミモミのＴシャツ、ハルとお揃いにしよ？画面の下の概要欄にリンクがあるから、チェックしてみてね！」
* **黄金レイアウト (オービーエス)**:
  * 画面の左側（60%）：アパレルＴシャツを着用したハルの実写動画を大きく表示。
  * 画面の右下（30%）：リスナーとのチャット欄。
  * 画面の右上：**「ミモミ公式ストアのQRコード」を常時表示**し、いつでも購入へ誘導。
...
gantt
    ...
    section ② ハル自律配信テスト（今日〜明日）
    アールブイシー（ＲＶＣ）音声モデル設定  :b1, 2026-05-18, 1d
    パイソン＆オービーエス動画連携テスト :b2, 2026-05-19, 1d
    ユーチューブ・ライブ 配信テスト       :b3, 2026-05-19, 1d
